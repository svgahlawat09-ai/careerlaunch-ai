...
export default function Internships() {
  ...
  const filteredListings = matchedInternships.filter((item) => {
    ...
    const matchesRegion = selectedRegion === 'All' || 
                          (selectedRegion === 'India' && (item.region === 'India' || item.location.toLowerCase().includes('india') || item.stipend.includes('₹'))) ||
                          (selectedRegion === 'Abroad' && (item.region === 'Abroad' || item.stipend.includes('$')));
    ...
    return matchesSearch && matchesRegion && matchesLocation && matchesCategory;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="glass-card rounded-2xl p-4 border border-white/10 grid grid-cols-1 sm:grid-cols-12 gap-3">
        ...
        <div className="sm:col-span-3">
          ...
          >
            <option value="All">🌍 All Regions (India & Abroad)</option>
            <option value="India">🇮🇳 India Companies & Hubs</option>
            <option value="Abroad">🌐 Abroad / Global Offers</option>
          </select>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredListings.map((internship) => {
          const logoUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(internship.logoSeed || internship.company)}`;
          const isApplied = state.myApplications?.some(a => a.internshipId === internship.id);
          const isIndia = internship.region === 'India' || internship.stipend.includes('₹') || internship.location.includes('Karnataka') || internship.location.includes('Maharashtra') || internship.location.includes('Haryana') || internship.location.includes('Telangana') || internship.location.includes('Tamil Nadu') || internship.location.includes('UP');
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <div>
                      <h3 className="font-bold text-white text-sm group-hover:text-rose-300 transition-colors flex items-center gap-1.5">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-extrabold text-[11px]">
                      {internship.matchPercentage}% Match
                    </span>
                    {/* Region Tag */}
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                      isIndia ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30'
                    }`}>
                      {isIndia ? '🇮🇳 India' : '🌐 Abroad'}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 text-[11px] text-slate-300">
                  ...
                  <span className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/5 flex items-center space-x-1">
                    ...
                    <span className="font-semibold text-emerald-300">{internship.stipend}</span>
                  </span>
                </div>
